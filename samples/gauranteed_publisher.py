...
class MessageDeliveryListenerImpl(MessageDeliveryListener):
    ...
    def on_delivery_receipt(self, delivery_receipt: 'DeliveryReceipt'):
        with lock:
            ...
            # print(f"\tMessage: {delivery_receipt.message}\n"
            print(f"\tIs persisted: {delivery_receipt.is_persisted}\n"
                  f"\tTimestamp: {delivery_receipt.time_stamp}\n"
                  f"\tException: {delivery_receipt.exception}\n")
            if delivery_receipt.user_context:
                print(f'\tUsercontext received: {delivery_receipt.user_context.get_custom_message}')
