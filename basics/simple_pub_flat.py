...
def direct_message_publish(messaging_service: pysolace.messaging.messaging_service.MessagingService, topic, message):
    try:
...
messaging_service = pysolace.messaging\
                    .messaging_service\
...
try: 
    while True:
        while rate <= MSG_RATE:
            ...
            topic = pysolace.messaging.utils.topic.Topic.of("taxinyc/ops/ride/called/v1" + f'/{rate}')
